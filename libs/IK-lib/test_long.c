...
int main(void)
{
	...
	for (times = 0; times < 1; times++) {
		double q0[] = {0.0, 0.0, 0.0, 0.0};
		printf("Fwd for q0: {%f, %f, %f, %f}\n", q0[0], q0[1], q0[2], q0[3]);

		double out_tcp[4][4];
		double orient_angle;
		if (drive_to_cart(&kin_model_long, q0, out_tcp, &orient_angle) < 0) {
		...
		if (drive_to_cart(&kin_model_long, q, out_tcp, &orient_angle) < 0) {
			printf("drive_to_cart() failed\n");
			return -1;
		}
		...
		drive_to_joint_full(&kin_model_long, q, th, &res_q_extra, &ax4_fwd_ret);
		printf("res_q_extra:\n");
		printf("q23: %f\n", res_q_extra.q23);
		...
		if (cart_to_drive(&kin_model_long, inv_in, orient_angle, q_out) < 0) {
