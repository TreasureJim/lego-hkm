...
private:
  ...
  Eigen::Vector3d find_plane_normal(Eigen::Vector3d p1, Eigen::Vector3d p2,
                                    Eigen::Vector3d p3) {
  ...
  Eigen::Vector2d find_circle_params(Eigen::Vector2d p1, Eigen::Vector2d p2,
                                     Eigen::Vector2d p3) {
  ...
  void find_circle_centre_radius_3d() {
    ...
    this->normal =
        find_plane_normal(this->points[0], this->points[1], this->points[2]);
    ...
    this->plane_vec2 =
        normal.cross(this->plane_vec1) / normal.cross(this->plane_vec1).norm();

    // desmos visualisation: https://www.desmos.com/3d/chdszznbfw
    auto to_2d = [&](Eigen::Vector3d point) -> Eigen::Vector2d {
      return Eigen::Vector2d((point - origin).dot(this->plane_vec1),
                             (point - origin).dot(this->plane_vec2));
    };
    ...
    this->origin_3d = origin + centre_2d[0] * this->plane_vec1 +
                      centre_2d[1] * this->plane_vec2;
  }
  ...
  void paramtric_arc_3d() {
    ...
    if (angle_12 + angle_23 > angle_23 + angle_31 and
        angle_12 + angle_23 > angle_31 + angle_12)
      order = {v1, v2, v3};
